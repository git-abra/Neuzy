...
class TestingCompleteModel():
    def __init__(self, filepath, line, model):
        ...
        with open (self.filepath) as f:
            reader = csv.reader(f)
            testdata = list(reader)[self.line - 1]         # select line
            testdata = [float(ele) for ele in testdata]
            testdata = np.array(testdata)
        # testdata = [float(ele) for ele in testdata]
        self.testdata = fnc.removeNans(testdata)

    def testData(self, populationsize):
        # TODO
        self.line = 1
        testingfinaldata = TestingCompleteModel("./paropt/datadump/parameter_values/best10_par.csv", line = self.line)

        self.model.run(populationsize, testing = False)   # testing flag if testingfinaldata is wanted or if it should proceed to random initialization

class ATPCalc(TestingCompleteModel):
    def __init__(self, model):
        ...
        self.atpmodel = HocModel(   "To21_nap_strong_trunk_together.hoc",
    ...
    def calcATP(self):              
        ...
        for i in range(1,214,1):   # range 1,11,1
            ...
            testingfinaldata = TestingCompleteModel("./paropt/data/parameter_values/final_par.csv", line = i)
            ...
            init_data, indices = self.atpmodel.getMechanismItems() # getting the indices of the parameter combination for "To21_nap_strong_trunk_together.hoc"

            # update original cell
            self.atpmodel.updateAutoParameters(testdata, indices)

            ## Stimulation
            stim = h.IClamp(self.atpmodel.mycell.soma[0](0.5)) # stim at soma
            stim.delay = self.atpmodel.delay
            stim.dur = self.atpmodel.duration
            stim.amp = 0.8
            ...
            for sl in cs.SL_NAMES:
                inputsl = getattr(self.atpmodel.mycell, sl)
                for sec in inputsl:
                    for seg in sec:
        ...
        with open("./paropt/models/ATP/qna_qca_atp.json", 'w') as f:
            json.dump(output_dict, f, indent = 4, separators = (',  \n', ': '))


        with open("./paropt/models/ATP/atp.csv", 'w') as f:
