...
for r            = 1 : numel (regions)
    ...
    if tot_nsyn  > numel (nodes{r})
        ...
        synids1  = datasample (nodes{r}, (tot_nsyn - extra_nodes), ...
            'Replace',         false);
        synids2  = datasample (nodes{r},  extra_nodes, ...
            'Replace',         false);
        synids{r} = [synids1; synids2];
    else
        synids{r} = datasample (nodes{r}, tot_nsyn, ...
            'Replace',         false);
    end
...
if syndens       > (syn_dens + 0.0002)
    while syndens > (syn_dens + 0.0002)
        syn_ids  = datasample (syn_ids, (numel (syn_ids) - 1), ...
            'Replace',         false);
        syndens  = numel (syn_ids) / (sum (region_len));
    end
elseif syndens   < (syn_dens - 0.0002)
    while syndens < (syn_dens - 0.0002)
        toadd    = datasample (not_synids, 1, ...
            'Replace',         false);
