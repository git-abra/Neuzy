function [neuronn,tree] = poiraziCh(inputtree, morphopath)
...
for ctr = 1:length(tree{t}.rnames)    
    if strcmp(tree{t}.rnames{ctr},'Apical')
        Apind = ctr;
        tree{t}.rnames{ctr} = 'apical';
    elseif strcmp(tree{t}.rnames{ctr},'Dendrite')
        Baind = ctr;
        tree{t}.rnames{ctr} = 'basal';
    elseif strcmp(tree{t}.rnames{ctr},'CellBody')
        Soind = ctr;
        tree{t}.rnames{ctr} = 'soma';
    elseif strcmp(tree{t}.rnames{ctr},'Axon')
        Axind = ctr;
        tree{t}.rnames{ctr} = 'axon';
    elseif strcmp(tree{t}.rnames{ctr},'MainApical')
        MApind = ctr;
        MainAp = find(tree{t}.R == MApind);
    end
end

if exist('Apind') & exist('MApind')
    tree{t}.R(MainAp) = Apind;
end

OrBaNodes = find(tree{t}.R == Baind);
tree{t} = CA1pyramidalcell_sort_PP(tree{t},'-p',1);
    tree{t}.R(OrBaNodes) = 2;
    % % figure; hold on;
    % % plot_tree(tree{t},tree{t}.R)
    % % soNoodes = find(tree{t}.R == 1);
    % % baNoodes = find(tree{t}.R == 2);
    % % trNoodes = find(tree{t}.R == 3);
    % % ptNoodes = find(tree{t}.R == 4);
    % % apNoodes = find(tree{t}.R == 5);
    % % scatter3(tree{t}.X(soNoodes),tree{t}.Y(soNoodes),...
    % %     tree{t}.Z(soNoodes),'filled','g');
    % % scatter3(tree{t}.X(baNoodes),tree{t}.Y(baNoodes),...
    % %     tree{t}.Z(baNoodes),'filled','r');
    % % scatter3(tree{t}.X(trNoodes),tree{t}.Y(trNoodes),...
    % %     tree{t}.Z(trNoodes),'filled','k');
    % % scatter3(tree{t}.X(ptNoodes),tree{t}.Y(ptNoodes),...
    % %     tree{t}.Z(ptNoodes),'filled','b');
    % % scatter3(tree{t}.X(apNoodes),tree{t}.Y(apNoodes),...
    % %     tree{t}.Z(apNoodes),'filled','y');

    % NrRegions = length(tree{t}.rnames);
    % if ~exist('Axind')
    %     tree{t}.rnames{NrRegions+1} = 'axon';
    %     tree{t}.rnames{NrRegions+2} = 'trunk';
    %     tree{t}.rnames{NrRegions+3} = 'peritrunk';
    % else
    %     tree{t}.rnames{NrRegions+1} = 'trunk';
    %     tree{t}.rnames{NrRegions+2} = 'peritrunk';
    % end
    % % tree{t}.rnames = {'apical','soma','basal','axon','trunk','peritrunk'};
    % apnodes = find(tree{t}.R == Apind);
    % trunknodes = find(tree{t}.X(apnodes) <= 50);
    % if ~isempty(trunknodes)
    %     tree{t}.R(apnodes(trunknodes)) = 5;
    % end
    % peritrunknodes = find(tree{t}.X(apnodes) > 50 & tree{t}.X(apnodes) <= 300);
    % if ~isempty(peritrunknodes)
    %     tree{t}.R(apnodes(peritrunknodes)) = 6;
    % end
    % % plot_tree(tree{t},tree{t}.R);
    
fname = tree{t}.name;
tree{t} = t2n_writeTrees(tree{t}, [], fullfile (morphopath, strcat(fname,'_', t, '.mtr')));
...
[basalnode, apicalnode] = calculate_basalapicalnode(tree{t});

[trunk_branchpoints, peritrunk_branch] = find_peritrunknodes(tree{t});

[idnodes300, idnodes350] = getdistalnodesPP(tree{t});

treenodes = (1:numel(tree{t}.X))';


% ******************* Scaling the original sigmoid from Poirazi's model:
Rm_sigmoid        = getPPsigmoid (tree{t}, Rm_default, Rm_end, Rm_dhalf, Rm_steep);
Ri_sigmoid       = getPPsigmoid (tree{t}, Ri_default, Ri_end, Ri_dhalf, Ri_steep);
...
decay_end        = 300;
%---------------------
ar2_hha_old_linear   = getPPar2        (tree{t}, ...
...
if insertka
    ...
    [gkabar_kap_fixed, gkabar_kad_linear] = getPPka (tree{t}, ...
...
% -----------
[gkbar_cagk, gbar_kca] = getPPkca (tree{t}, ...
soma_kca,soma_cagk, ...
kca_distal_distance);
% gkbar_cagk = 1;
% gbar_kca = 1;
