...
% DIFFERENT WAY:
trunk_nodes = tree.R == find(strcmp(tree.rnames,'trunk'));                 % vector, "1" for row using trunk region in tree.R
ntrunkn = sum(trunk_nodes)                                                 % number of nodes in the trunk
% The maximum vertical distance of the trunk from the Poirazi's model is
% 423.75 ~= 424 um
apical_vertdist_PP = 156.96;
basal_vertdist_PP = 52.53; % um

step = 423.75/ntrunkn; % calculate the size of the step
dist = 0+step:step:423.75; % 424 = max trunk vertical disance from Poirazi
perpdist = abs(tree.Y)-tree.Y(1);
treenodes = (1:numel(tree.X))';
perpdist_trunk = perpdist.*trunk_nodes;  % warum??? alles mal 1 multiplizieren lol
